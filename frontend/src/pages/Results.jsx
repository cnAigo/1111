...
export default function Results({ testResults, caseDetails }) {
  ...
  // Class-level filter: only show classes matching the selected filter
  ...
  // Case-level filter: within each class, only show matching cases
  const getVisibleCases = (cases, f) => {
    if (!cases) return [];
    if (f === 'all') return cases;
    if (f === 'skipped') return cases; // skipped not individually listed
    ...
    <div className="flex-1 overflow-y-auto mx-8 mb-6 bg-white rounded-xl border border-slate-200 shadow-sm">
      {testResults.length === 0 ? <Empty icon={CheckCircle2} title="暂无结果" sub="执行测试后结果将显示在这里" /> : (
        <div className="flex flex-col h-full">
          {/* Sticky header: Summary + Filters */}
          <div className="sticky top-0 z-10 bg-white rounded-t-xl px-5 pt-5 pb-3 border-b border-slate-100">
            {/* Summary */}
