...
const onSearch = async (value) => {
  ...
  try {
    const response = await api.get('/api/api-testing/collections/search', {
      params: {
...
const loadCollections = async (projectId) => {
  try {
    const response = await api.get('/api/api-testing/collections/', {
      params: {
        project: projectId
      }
    })
...
const loadRequests = async () => {
  ...
  try {
    const response = await api.get('/api/api-testing/requests/')
...
const onNodeClick = async (data) => {
  if (data.type === 'request') {
    try {
      const apiResponse = await api.get(`/api/api-testing/requests/${data.id}/`)
...
const deleteNode = () => {
  ...
  ElMessageBox.confirm(
    ...
    try {
      if (node.type === 'collection') {
        await api.delete(`/api/api-testing/collections/${node.id}/`)
      } else {
        await api.delete(`/api/api-testing/requests/${node.id}/`)
      }
...
const saveCollectionName = async () => {
  ...
  try {
    await api.patch(`/api/api-testing/collections/${editingNodeId.value}/`, {
...
const sendRequest = async () => {
  ...
  try {
    ...
    const apiResponse = await api.post(`/api/api-testing/requests/${selectedRequest.value.id}/execute/`, requestData)
...
const saveRequest = async () => {
  ...
  try {
    ...
    if (selectedRequest.value.id) {
      response = await api.patch(`/api/api-testing/requests/${selectedRequest.value.id}/`, requestData)
    } else {
      response = await api.post('/api/api-testing/requests/', requestData)
    }
...
const createCollection = async () => {
  ...
  try {
    const response = await api.post('/api/api-testing/collections/', {
      ...collectionForm,
      project: selectedProject.value
    })
...
const updateCollection = async () => {
  ...
  try {
    await api.patch(`/api/api-testing/collections/${editCollectionForm.id}/`, editCollectionForm)
