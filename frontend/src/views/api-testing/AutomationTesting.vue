          ...
          <div class="suite-header">
            <div class="suite-title">
              ...
              <div class="suite-actions">
                <el-button type="success" size="large" @click="runTestSuite(selectedSuite)" :loading="running" class="run-suite-btn">
...
const loadTestSuites = async () => {
  ...
  try {
    const response = await api.get('/api/api-testing/test-suites/', {
...
const loadEnvironments = async () => {
  try {
    // 获取全局环境 + 当前项目环境
    const response = await api.get('/api/api-testing/environments/', {
...
const loadRequestTree = async () => {
  ...
  try {
    // 加载集合
    const collectionsRes = await api.get('/api/api-testing/collections/', {
      params: { project: selectedProject.value }
    })
    const collections = collectionsRes.data.results || collectionsRes.data

    // 加载请求
    const requestsRes = await api.get('/api/api-testing/requests/')
...
const loadExecutions = async () => {
  ...
  try {
    const response = await api.get('/api/api-testing/test-executions/', {
...
const selectSuite = async (suite) => {
  ...
  // Load suite requests into the suite object
  await loadSuiteRequests()
  loadExecutions()
}

const loadSuiteRequests = async () => {
  if (!selectedSuite.value) return
  try {
    const response = await api.get(`/api/api-testing/test-suites/${selectedSuite.value.id}/requests/`)
    selectedSuite.value.suite_requests = response.data || []
  } catch (error) {
...
const runTestSuite = async (suite) => {
  running.value = true
  try {
    const response = await api.post(`/api/api-testing/test-suites/${suite.id}/execute/`)
...
const duplicateSuite = async (suite) => {
  try {
    ...
    await api.post('/api/api-testing/test-suites/', newSuite)
...
const deleteSuite = async (suite) => {
  try {
    ...
    await api.delete(`/api/api-testing/test-suites/${suite.id}/`)
...
const submitSuiteForm = async () => {
  ...
  try {
    if (editingSuite.value) {
      await api.patch(`/api/api-testing/test-suites/${editingSuite.value.id}/`, suiteForm)
      ElMessage.success(t('apiTesting.messages.success.suiteUpdated'))
    } else {
      await api.post('/api/api-testing/test-suites/', suiteForm)
...
const addSelectedRequests = async () => {
  ...
  try {
    // 这里需要调用添加请求到套件的API
    await api.post(`/api/api-testing/test-suites/${selectedSuite.value.id}/add-requests/`, {
...
const updateRequestEnabled = async (suiteRequest) => {
  try {
    await api.patch(`/api/api-testing/test-suites/${selectedSuite.value.id}/requests/${suiteRequest.request_id}/`, {
...
const removeRequest = async (suiteRequest) => {
  try {
    ...
    await api.delete(`/api/api-testing/test-suites/${selectedSuite.value.id}/requests/${suiteRequest.request_id}/`)
...
const reloadCurrentSuite = async () => {
  if (!selectedSuite.value) return
  try {
    // Reload suite from list and reload its requests
    const index = testSuites.value.findIndex(s => s.id === selectedSuite.value.id)
    if (index !== -1) {
      selectedSuite.value = { ...testSuites.value[index] }
    }
    await loadSuiteRequests()
  } catch (error) {
