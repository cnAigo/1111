...
const getStatusType = (status) => {
  ...
  if (s === 'STOPPED' || s === 'ABORTED' || s === 'CANCELLED') return 'info'
  return 'info'
}

const getStatusText = (status) => {
  const s = (status || '').toUpperCase()
  if (s === 'PASSED' || s === 'COMPLETED' || s === 'SUCCESS') return t('uiAutomation.report.statusSuccess')
  if (s === 'FAILED') return t('uiAutomation.report.statusFailed')
  if (s === 'RUNNING') return t('uiAutomation.report.statusRunning')
  if (s === 'PENDING') return t('uiAutomation.report.statusPending')
  if (s === 'STOPPED' || s === 'ABORTED' || s === 'CANCELLED') return t('uiAutomation.report.statusAborted')
  return status || ''
}
