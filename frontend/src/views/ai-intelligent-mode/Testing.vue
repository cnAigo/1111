<template>
  <div class="page-container">
        ...
        <el-col :span="12">
          ...
          <el-form :model="taskForm" label-position="top">
            <el-form-item style="margin-bottom:8px">
              <el-select v-model="selectedCaseId" placeholder="从已有用例加载" clearable @change="loadCase" style="width:100%">
            ...
            <el-form-item style="margin-bottom:0">
              <el-button type="primary" @click="handleRun" :loading="running" :disabled="!taskForm.description">
                <el-icon><VideoPlay /></el-icon> 执行
              </el-button>
              <el-button type="danger" @click="handleStop" :disabled="!running || analyzing" v-if="running">
                <el-icon><SwitchButton /></el-icon> 停止
              </el-button>
              <el-button type="success" @click="handleSaveAsCase" :disabled="!taskForm.description">
                <el-icon><DocumentAdd /></el-icon> 保存
              </el-button>
              <el-button type="warning" @click="handlePolish" :loading="polishing" :disabled="!taskForm.description">
                <el-icon><MagicStick /></el-icon> AI润色
              </el-button>
              <el-button @click="showPrompts = true">
                <el-icon><EditPen /></el-icon> 提示词
              </el-button>
              <el-button @click="showRecorder = true">
                <el-icon><FolderOpened /></el-icon> 脚本库
              </el-button>
            </el-form-item>
          </el-form>
    ...
    <!-- 提示词管理侧边栏 -->
    <PromptManager v-model="showPrompts" />
    <!-- 操作脚本库 -->
    <StepRecorder v-model="showRecorder" />
  </div>
</template>
...
import { VideoPlay, DocumentAdd, CircleCheckFilled, CircleCheck, Loading, SwitchButton, MagicStick, EditPen, FolderOpened } from '@element-plus/icons-vue'
...
import StepRecorder from './StepRecorder.vue'
...
const loadCaseList = async () => {
  try { const r = await getAICases(); caseList.value = r.data?.results || [] } catch (e) { /* ignore */ }
}
onMounted(loadCaseList)

// 从下拉框加载用例到输入框
const loadCase = (id) => {
  if (!id) { taskForm.description = ''; return }
  const c = caseList.value.find(x => x.id === id)
  if (c) taskForm.description = c.task_description || ''
}
...
const handlePolish = async () => {
  ...
  try {
    const r = await request.post('/api/ui-automation/ai-cases/polish/', { text: taskForm.description })
    const polished = r.data?.polished || r.data
    if (polished && typeof polished === 'string') {
      taskForm.description = polished.trim()
      ElMessage.success('润色完成')
    }
  } catch (e) {
    ElMessage.error('润色失败: ' + (e.response?.data?.error || e.message))
  } finally {
...
const handleRun = async () => {
  ...
  try {
    ...
    const execId = response.data.execution_id || response.data.id
    if (!execId) { ElMessage.error('启动失败: 未获取到执行ID'); running.value = false; return }
    currentExecutionId.value = execId
    localStorage.setItem('ai_current_execution_id', String(execId))
    localStorage.setItem('ai_task_description', taskForm.description)
    ...
    localStorage.removeItem('ai_current_execution_id')
  }
}
...
// Clear any stale execution state on mount
onMounted(() => {
  localStorage.removeItem('ai_current_execution_id')
  localStorage.removeItem('ai_task_description')
})
onUnmounted(() => {
  // Keep the timer running even when navigating away
  // State saved in localStorage above
})
</script>
