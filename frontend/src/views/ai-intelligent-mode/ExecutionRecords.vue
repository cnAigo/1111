  ...
  <div class="page-container">
    ...
    <div class="card-container">
      ...
      >
        ...
        <el-table-column :label="$t('uiAutomation.ai.executionRecords.executor')" width="100">
          <template #default>{{ '—' }}</template>
        </el-table-column>
        <el-table-column :label="$t('uiAutomation.common.operation')" width="240" fixed="right">
          <template #default="{ row }">
            <el-button size="small" @click="viewDetail(row)" style="margin:0 2px">详情</el-button>
            <el-button size="small" type="success" @click="viewReport(row)" style="margin:0 2px">报告</el-button>
            <el-button size="small" type="danger" @click="deleteSingle(row)" style="margin:0 2px">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="showDetailDialog" :title="$t('uiAutomation.ai.executionRecords.executionDetail')" width="800px">
      <div v-if="currentRecord" class="record-detail">
        ...
        <!-- 执行截图 -->
        <div v-if="screenshotList.length > 0" class="detail-item mt-15">
          <span class="label">执行截图:</span>
        </div>
        <div v-if="screenshotList.length > 0" style="display:flex;flex-wrap:wrap;gap:8px;margin-top:8px">
          <img v-for="(s, i) in screenshotList" :key="i" :src="'/' + s"
               style="max-width:200px;max-height:150px;border:1px solid #eee;border-radius:4px;cursor:pointer"
               @click="previewImg = '/' + s; showPreview = true" />
        </div>
        <el-dialog v-model="showPreview" title="截图预览" width="80%">
...
import { getAIExecutionRecords, batchDeleteAIExecutionRecords } from '@/api/ui-automation'
...
const screenshotList = computed(() => {
  if (!currentRecord.value?.screenshots) return []
  try { return JSON.parse(currentRecord.value.screenshots) } catch (e) { return [] }
})
...
const deleteSingle = async (row) => {
  try {
    ...
    await batchDeleteAIExecutionRecords([row.id])
