        ...
        <el-card class="category-card">
          <template #header>
            <div class="category-header">
              ...
              <span class="category-title">{{ category.name || getCategoryName(category.category) }}</span>
...
const getCategoryName = (category) => {
  if (!category) return ''
  const translated = t(`dataFactory.scenarios.${category}`)
  if (translated && translated !== `dataFactory.scenarios.${category}`) return translated
  return category
}

const getScenarioDesc = (scenario) => {
  if (!scenario) return ''
  const translated = t(`dataFactory.scenarioDescs.${scenario}`)
  if (translated && translated !== `dataFactory.scenarioDescs.${scenario}`) return translated
  return ''
}

const getToolDisplayName = (toolName) => {
  const translated = t(`dataFactory.tools.${toolName}`)
  if (translated && translated !== `dataFactory.tools.${toolName}`) return translated
  return getToolDisplayNameOld(toolName) || toolName || ''
}
...
const getToolDescription = (toolName) => {
  const translated = t(`dataFactory.toolDescs.${toolName}`)
  if (translated && translated !== `dataFactory.toolDescs.${toolName}`) return translated
  return getToolDescriptionOld(toolName) || ''
}
