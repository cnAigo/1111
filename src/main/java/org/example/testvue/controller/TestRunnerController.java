...
public class TestRunnerController {
    ...
    /**
     * Parse the JaCoCo XML report and return line- and branch-coverage
     * percentages.  The report is expected at {@code target/site/jacoco/jacoco.xml}
     * — generated by the standard {@code mvn test jacoco:report} pipeline.
     *
     * Returns {@code available: false} with a descriptive message when the
     * report file is missing or unparseable, so the frontend can degrade
     * gracefully instead of showing a spinner forever.
     */
    @GetMapping("/coverage")
    public Map<String, Object> getCoverage() {
        Map<String, Object> result = new LinkedHashMap<>();
        Path jacocoXml = Paths.get(System.getProperty("user.dir"), "target", "site", "jacoco", "jacoco.xml");
        if (!Files.exists(jacocoXml)) {
            result.put("available", false);
            result.put("message", "JaCoCo report not found. Run mvn test jacoco:report first.");
            return result;
        }
        try {
            DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
            Document doc = dbf.newDocumentBuilder().parse(jacocoXml.toFile());
            ...
            for (int i = 0; i < counters.getLength(); i++) {
                ...
                if ("LINE".equals(type))      { lineCovered   = covered; lineMissed   = missed; }
                else if ("BRANCH".equals(type)) { branchCovered = covered; branchMissed = missed; }
            }

            int lineTotal   = lineCovered + lineMissed;
            int branchTotal = branchCovered + branchMissed;

            // Round to one decimal place so the frontend displays clean percentages
            result.put("available", true);
            result.put("lineCoverage",   lineTotal   > 0 ? Math.round(1000.0 * lineCovered   / lineTotal)   / 10.0 : 0.0);
            result.put("lineCovered",    lineCovered);
            result.put("lineMissed",     lineMissed);
            result.put("lineTotal",      lineTotal);
            result.put("branchCoverage", branchTotal > 0 ? Math.round(1000.0 * branchCovered / branchTotal) / 10.0 : 0.0);
            result.put("branchCovered",  branchCovered);
            result.put("branchMissed",   branchMissed);
            result.put("branchTotal",    branchTotal);
        } catch (Exception e) {
            result.put("available", false);
            result.put("message", "Failed to parse JaCoCo report: " + e.getMessage());
        }
        return result;
    }
