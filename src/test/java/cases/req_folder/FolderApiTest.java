...
public class FolderApiTest extends ApiTestHelper {
    ...
    void test_GNYL_012_createFolderUnderRoot() {
        ...
        log.info("GNYL_012 通过: 新建文件夹 [{}]", folderName);
    }
    ...
    void test_GNYL_013_createSubFolder() {
        String parentId = api.createFolder(PROJECT_ID, PROJECT_ID);
    ...
    void test_GNYL_016_createSiblingFolder() {
        ...
        String child1Id = api.createFolder(PROJECT_ID, parentId);
        api.renameFolder(PROJECT_ID, child1Id, parentId, "AT_Child1_" + suffix());

        String child2Id = api.createFolder(PROJECT_ID, parentId);
    ...
    void test_GNYL_018_renameFolderValid() {
        ...
        String treeResp = api.getTree(PROJECT_ID, PROJECT_ID);
        Assertions.assertTrue(treeResp.contains(newName), "树结构中应包含新名称: " + newName);
        Assertions.assertFalse(treeResp.contains(oldName), "树结构中不应包含旧名称: " + oldName);
    ...
    void test_GNYL_020_renameFolderDuplicate() {
        String folderId1 = api.createFolder(PROJECT_ID, PROJECT_ID);
        String name1 = "AT_Dup1_" + suffix();
        api.renameFolder(PROJECT_ID, folderId1, PROJECT_ID, name1);

        String folderId2 = api.createFolder(PROJECT_ID, PROJECT_ID);
        ...
        JsonObject root = JsonParser.parseString(resp).getAsJsonObject();
        int code = root.get("code").getAsInt();
        assertRejected(resp, "重命名为重复名称应失败");
    ...
    void test_renameNonExistingFolder() {
        ...
        JsonObject root = JsonParser.parseString(resp).getAsJsonObject();
        int code = root.get("code").getAsInt();
        assertRejected(resp, "不存在的文件夹重命名应失败");
    ...
    void test_multiLevelDeleteRecoverClean() {
        String grandParentId = api.createFolder(PROJECT_ID, PROJECT_ID);
        String grandParentName = "AT_GrandParent_" + suffix();
        api.renameFolder(PROJECT_ID, grandParentId, PROJECT_ID, grandParentName);

        String parentId = api.createFolder(PROJECT_ID, grandParentId);
        String parentName = "AT_Parent_" + suffix();
        ...
        String treeResp = api.getTree(PROJECT_ID, PROJECT_ID);
        Assertions.assertTrue(treeResp.contains(grandParentName), "恢复后树中应有顶层文件夹");
        log.info("多级恢复完成");
        ...
        Assertions.assertFalse(treeResp.contains(grandParentName), "彻底删除后树中不应有顶层文件夹");
        log.info("多级彻底删除完成");
    }
    ...
    void test_deleteNonExistingFolder() {
        ...
        JsonObject root = JsonParser.parseString(resp).getAsJsonObject();
        int code = root.get("code").getAsInt();
        assertRejected(resp, "删除不存在的文件夹应失败");
    ...
    void test_recoverNonExistingFolder() {
        ...
        JsonObject root = JsonParser.parseString(resp).getAsJsonObject();
        int code = root.get("code").getAsInt();
        assertRejected(resp, "恢复不存在的文件夹应失败");
    ...
    void test_searchFolderChildren() {
        ...
        String childId = api.createFolder(PROJECT_ID, folderId);
        String childName = "AT_Child_" + suffix();
