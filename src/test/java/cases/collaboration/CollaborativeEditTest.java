...
public class CollaborativeEditTest extends BaseTest {
    ...
    private String[] createTempDocWithReqItem() {
        String[] doc = createTempDoc();
        api.addReqItem(PROJECT_ID, doc[0], doc[0]);
        return doc;
    }

    private void openEditorAndClickContent(String docName) {
        reqPage.doubleClickTreeNode(docName);
    ...
    public void test_GNYL_197() {
        ...
        try {
            reqPage.refreshTree();
            page.waitForTimeout(1000);
            openEditorAndClickContent(doc[1]);
    ...
    public void test_GNYL_238() {
        ...
        String[] doc2 = createTempDoc();
        try {
            reqPage.refreshTree();
            page.waitForTimeout(1000);

            reqPage.rightClickTreeNode(doc1[1]);
            page.waitForTimeout(500);
            page.getByText("打开", new Page.GetByTextOptions().setExact(true)).click();
            page.waitForTimeout(300);
            page.getByText("共享模式", new Page.GetByTextOptions().setExact(true)).click();
            ...
            reqPage.rightClickTreeNode(doc2[1]);
            page.waitForTimeout(500);
            page.getByText("打开", new Page.GetByTextOptions().setExact(true)).click();
            page.waitForTimeout(300);
            page.getByText("共享模式", new Page.GetByTextOptions().setExact(true)).click();
            ...
            cleanupDoc(doc1[0], doc1[2]);
            cleanupDoc(doc2[0], doc2[2]);
        }
    }
